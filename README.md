# Welcome to My Portfolio

> **Building a comprehensive digital professional presence through code, documentation, and thought leadership.**

## 🎯 Purpose

This repository serves dual purposes:
1. **Navigation Hub:** Easy access to my technical work and professional profiles
2. **Strategic Documentation:** A wiki resource demonstrating software house operations, best practices, and technical expertise

## 🚀 Quick Links

### Code Repositories
- **[Featured Projects](https://github.com/gezielcarvalho)** - Highlighted repositories on my homepage
- **[All Repositories](https://github.com/gezielcarvalho?tab=repositories)** - Complete portfolio of technical work

### Documentation & Learning
- **[Wiki](https://github.com/gezielcarvalho/portfolio/wiki)** - Comprehensive documentation on software house operations, architecture, DevOps, and best practices

## 📚 What's Inside

### The Wiki
Detailed technical documentation covering:
- **Software House Operations:** Structure, workflows, and client management
- **Technical Architecture:** Design patterns, scalability, and system design
- **DevOps Practices:** CI/CD, infrastructure, monitoring, and deployment
- **Best Practices:** Code standards, testing strategies, and documentation
- **Learning Resources:** Career development, tools, and recommended materials

## 🎓 For Different Audiences

### 👨‍💼 For Recruiters
- Browse my [pinned repositories](https://github.com/gezielcarvalho) for technical capabilities
- Review the wiki for depth of knowledge and documentation skills
- Check my [LinkedIn profile](#) for professional experience

### 💼 For Customers/Clients
- Explore repositories to see technical solutions and code quality
- Read wiki sections on software house operations and best practices
- See how I approach problem-solving and documentation

### 🧑‍🎓 For Mentees
- Use the wiki as a learning resource for software development
- Study real-world examples and best practices
- Learn about career development in tech

## 📖 Getting Started

1. **New here?** Start with the [wiki home page](https://github.com/gezielcarvalho/portfolio/wiki)
2. **Looking for code?** Check out my [featured repositories](https://github.com/gezielcarvalho)
3. **Interested in collaboration?** See contact information below

## 🔗 Connect With Me

- [**LinkedIn:**](https://www.linkedin.com/in/gezielcarvalho/)
- [**Personal Website:**](https://gezielcarvalho.info/)
- [**Company Blog:**](https://sabresoftware.com.br/)

## 📝 Repository Structure

```
portfolio/
├── README.md                              # This file - overview and navigation
└── wiki/                                  # (GitHub Wiki) Technical documentation
    ├── Home
    ├── Software House Operations
    ├── Technical Architecture
    ├── DevOps & Infrastructure
    ├── Best Practices & Standards
    └── Resources & Learning Paths
```

## 🎯 Current Focus

- ✅ Building comprehensive wiki documentation
- 🔄 Establishing consistent content creation workflow
- 📊 Growing professional visibility across platforms
- 🤝 Engaging with developer community and mentees

---

**Last Updated:** November 23, 2025  
**Status:** Active development and continuous improvement

> *This repository is a living project that evolves with my professional journey. Star it to follow along!*
